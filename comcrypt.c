...
PROCESS_THREAD(comcrypt_process, ev, data)
{
  ...
  for (i = 0; i < SIGNAL_LEN; i++)
  {
    LOG_INFO_("%08x", (unsigned int)signal[i].full);
  }
  LOG_INFO_("\n");
  ...
  {
    signal_bytes[i + 0] = (uint8_t)((signal[i >> 2].full & 0xFF000000) >> 24);
    signal_bytes[i + 1] = (uint8_t)((signal[i >> 2].full & 0x00FF0000) >> 16);
    signal_bytes[i + 2] = (uint8_t)((signal[i >> 2].full & 0x0000FF00) >> 8);
    signal_bytes[i + 3] = (uint8_t)((signal[i >> 2].full & 0x000000FF) >> 0);
  }
